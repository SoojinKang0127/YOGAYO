	...
	<%}%>
		let randomNums = [];
					while (numArr.length > 0) { // 추출을 원하는 갯수
						let target = numArr.splice(Math.floor(Math.random()
								* numArr.length), 1)[0];
						randomNums.push(target);
					}
					for (var i = 0; i < 23; i++) {
						$(".interests").append(
								'<span class="interest">' + randomNums[i]
										+ '</span>')
					}
				})
	</script>
			...
			<div class="sign-up-form">
				<form action="sign-up" method="POST" enctype="multipart/form-data">
					...
					<div class="profile_pic_wrapper">
						<label for="profile" class="profile_pic_icon" id="icon">
							<img class="thumbnail" >
						</label> <input
							type="file" id="profile" name="profile" class="profile_input" onchange="setThumbnail(event);">
					</div>
					<label for="">이메일</label> <input type="text" name="id" id="email"
						placeholder="johndoe@example.com" /> <br />
					<div class="msgContainer">
						<label for=""></label><span class="emailVaildMsg">이미
							사용중이거나 올바르지 않은 이메일입니다.</span><br />
					</div>
					<label for="">이름</label> <input type="text" name="name" id="name"
						placeholder="3자 이상" /><br /> <label for="">생년월일</label> <input
						type="date" name="bDate" id="birthDay" placeholder="" /><br /> <label
						for="">성별</label> <select name="gender" id="gender">
						<option value="M">남성</option>
						<option value="F">여성</option>
					</select> <br /> <label for="">숙련도</label> <select name="profiLevel"
						id="profiLevel">
		...
		<section id="step_2">
			...
			<div id="signUpBtn">
				<input type="submit" value="가입하기" disabled="disabled" id="submitBtn"
					onclick="alert('회원가입되었습니다.')" class="submitBtn">
			</div>
			</form>
